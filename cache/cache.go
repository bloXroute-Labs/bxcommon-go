...
func NewCache[K comparable, V any](hasher syncmap.Hasher[K], valueFetcher func(K) (*V, error), expDur time.Duration, cleanDur time.Duration) *Cache[K, V] {
	return newCache[K, V](hasher, valueFetcher, expDur, cleanDur, clock.RealClock{})
}

func newCache[K comparable, V any](hasher syncmap.Hasher[K], valueFetcher func(K) (*V, error), dur time.Duration, cleanDur time.Duration, clock clock.Clock) *Cache[K, V] {
	c := &Cache[K, V]{
