...
func NewSSLCertsFromFiles(privateCertFile string, privateKeyFile string, registrationOnlyCertFile string, registrationOnlyKeyFile string) SSLCerts {
	...
	registrationOnlyKeyBlock, err := os.ReadFile(registrationOnlyKeyFile)
	if err != nil {
	...
	privateCertBlock, certErr := os.ReadFile(privateCertFile)
	if certErr != nil {
		privateCertBlock = nil
	}
	privateKeyBlock, keyErr := os.ReadFile(privateKeyFile)
	if keyErr != nil {
		privateKeyBlock = nil
	}
	...
	if privateKeyBlock == nil && privateCertBlock == nil {
		...
		err = os.WriteFile(privateKeyFile, privateKeyBlock, 0644)
		if err != nil {
